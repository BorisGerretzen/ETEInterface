...
public static class DataLoaderFactory {
    ...
    public static DataSet GetDataSet(string path) {
        ...
        using (var stream = File.Open(path, FileMode.Open, FileAccess.Read)) {
            using (var reader = ExcelReaderFactory.CreateReader(stream)) {
                ...
                // configuration.UseColumnDataType = false;
                configuration.ConfigureDataTable = _ => new ExcelDataTableConfiguration {
                    UseHeaderRow = true
                };

                var result = reader.AsDataSet(configuration);
                if (result == null) throw new ArgumentException($"Could not read from '{path}', does the file exist?");
    ...
    public static DataLoader FromFile(string path, string sheetName, List<string> categoryHeaderNames) {
        ...
        if (dataSet.Tables[sheetName] == null) throw new ArgumentException($"Sheet '{sheetName}' does not exist in selected file.");
    ...
    public static List<DataLoader> FromAllSheets(string path, List<string> categoryHeaderNames) {
        ...
        foreach (DataTable? dataTable in dataSet.Tables) {
            if (dataTable == null) throw new ArgumentException($"Read null sheet from '{path}'.");
